...
Below are examples of color distribution histograms of each channel in BGR images of vehicle and non-vehicle. For certain cases, such as this one, vehicle and non-vehicle can be easily distinguished using histograms.

![alt text][histogram_of_color]

#### Spatial Binning

It is also sometimes useful to look at the color distribution for different color representations. Below is an example for the selected image. Consider HSL graph; yellow lane line is clearly identifiable. Black and white cars can also be identified: black car is in bottom left corner, and white car is in the top middle.
...
Using gradients is more robust way to identify cars in an image. It allows to accept a small variations of the car shape. Below is an example of HOG features captured for the selected image for each color channel in different color spaces.
...
All three types of features eventually become a one vertor of normalized features. The normalization code is contained in *Train Linear Support Vector Machine Classifier*->*Combine Features and Normalize*. Example of feature normalization is given below.
...
I tried various combinations of parameters and among combinations considered, the classifier trained with the following parameters shown the best performance on the training image set. It was a manual process---I achieved good results very quickly, that is why I did not try to automate parameter adjustment. 
```
